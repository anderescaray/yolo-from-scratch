...
def non_max_suppression(bboxes, iou_threshold, threshold, box_format="midpoint"):
    """
    It discards all the boxes with low probability. And with the survivers it takes
    the one with more probability and remove all other boxes with IoU > threshold
    with the same class
    ...
    for idx, (x, y) in enumerate(tqdm(loader)):
        ...
        for i in range(3):
            ...
            correct_class += torch.sum(
                torch.argmax(out[i][..., 5:][obj], dim=-1) == y[i][..., 5][obj].long()
            )
            tot_class_preds += torch.sum(obj)

            obj_preds = torch.sigmoid(out[i][..., 0]) > threshold
            correct_obj += torch.sum(obj_preds[obj])
