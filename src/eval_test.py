...
def evaluate_model(weights_path):
    ...
    # 1. Cargar la arquitectura (Tus 20 clases del supermercado)
    NUM_CLASSES = len(config.specific_class_labels)
    model = YOLOv4(num_classes=NUM_CLASSES).to(config.DEVICE)

    # 2. Cargar el cerebro (los pesos del checkpoint)
    ...
    # 3. Cargar el DataLoader de TEST
    # Usamos TEST_CSV en el lugar de val_csv_path
    _, test_loader, _ = get_loaders(
        train_csv_path=config.TRAIN_CSV, # Relleno
        val_csv_path=config.TEST_CSV,    # ¡El importante!
    ...
    # 4. Evaluación
    print("Calculando métricas en el set de Test (esto puede tardar unos minutos)...")
    with torch.no_grad():
